...
def _load_archive(directory: Path, run_id: str):
    ...
    return manifest, facts, candidates


@contextmanager
def repair_locks(root: Path, run_id: str):
    """Own the production writer locks across candidate recovery and formal replay."""
...
def repair(root: Path, run_id: str, *, apply: bool = False) -> dict:
    from .research_kpi import prepare_facts
    if not re.fullmatch(r"research_[A-Za-z0-9_-]+", run_id):
        raise ValueError("Invalid research run ID")
    root = root.resolve(strict=True)
    directory = root / "curation_data/research_runs" / run_id
    if not apply:
        manifest, facts, candidates = _load_archive(directory, run_id)
    ...
    with repair_locks(root, run_id):
        return _repair_locked(root, run_id)


def _repair_locked(root: Path, run_id: str) -> dict:
    """Internal replay; caller must continuously own ``repair_locks`` for this run."""
    ...
    directory = root / "curation_data/research_runs" / run_id
    # Read every archive input only after acquiring the writer locks. A prior
    # dry-run (or a just-finished review) must never overwrite newer decisions.
    manifest, facts, candidates = _load_archive(directory, run_id)
    before = audit_storage(root, facts, expected=len(facts))
    revised, preflight = prepare_facts(root, candidates, run_id, allow_replay=True)
