...
class WorkspaceTabsTests(unittest.TestCase):
    ...
    def test_competitor_workbench_starts_empty_and_uses_historical_data(self):
        ...
        self.assertIn("renderCompetitorInsightMarkdown(copy, insight)", SCRIPT)
        self.assertIn("renderCompetitorInsightMarkdown(copy, text)", SCRIPT)
        self.assertIn("window.markdownToHtml(markdown)", SCRIPT)
        self.assertIn('copy.className = "competitor-insight-markdown markdown-body"', SCRIPT)
        self.assertIn("is-raw-ai-output", SCRIPT)
        self.assertIn(".competitor-insight-markdown .chat-data-table", STYLE)
        self.assertIn(".competitor-insight-markdown h3", STYLE)
        self.assertNotIn("filter(Boolean).slice(0, 3)", SCRIPT)
        ...
        self.assertIn('setCompetitorInsightStatus(card, isAi ? ""', SCRIPT)
        self.assertNotIn("内网 AI 已完成真实生成", SCRIPT)
        self.assertIn("AI未生成：", SCRIPT)
