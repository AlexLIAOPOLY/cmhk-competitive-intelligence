# 全球五大运营商 2016–2025 数据库

## 入口

- `annual_metrics.json`：主数据和元数据。
- `annual_metrics.csv`：长表。
- `sources.json`：官方来源登记。
- `coverage.csv`：逐运营商、逐年、逐指标覆盖/缺口。
- `quality_audit.json` / `quality_audit.md`：质量门禁。
- `conflicts_and_scope_breaks.json` / `.csv`：重述、冲突、推导值与口径断点。
- `summary.md`：研究对象与使用边界。

## 与原数据库的关系

中国移动、中国电信、中国联通的财务数据继续以 `quarterly_competitor_metrics_2026-06-18/quarterly_metrics.json` 为唯一事实源；新增运营指标的规范侧表写入该原数据库目录的 `annual_operating_metrics_2016_2025.*`。本目录主表提供五家整合视图。Airtel 与 Jio 为新增主体，财务和运营数据均在本库。

## 重建

```bash
python3 scripts/build_global_top5_operator_database.py
```
