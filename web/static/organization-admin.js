(() => {
  ...
  function memberDetail(user) {
    ...
    const organization = `<dl class="organization-info-grid"><div><dt>所属部门</dt><dd>${esc(user.department || (user.authProvider === "feishu" ? "飞书未提供" : "—"))}</dd></div><div><dt>员工岗位</dt><dd><div class="organization-title-readonly" aria-label="员工岗位">${esc(user.title || (user.authProvider === "feishu" ? "飞书未提供" : "—"))}</div></dd></div></dl>`;
