(() => {
  ...
  function renderReceivedNewsTable(items) {
    ...
    target.innerHTML = `<div class="market-received-toolbar"><div><h2>新闻列表</h2><span>${selected ? `当前显示 ${filtered.length} / 本批次 ${items.length} 条` : `本批次共 ${items.length} 条`}${receivedAt ? ` · ${esc(receivedAt)}` : ""}</span></div><div class="market-time-filter" role="group" aria-label="按发布时间筛选新闻"><label><span>开始时间</span><input type="datetime-local" value="${esc(state.receivedStart)}" min="${esc(min)}" max="${esc(max)}" data-news-time-filter="start"></label><i aria-hidden="true">—</i><label><span>结束时间</span><input type="datetime-local" value="${esc(state.receivedEnd)}" min="${esc(min)}" max="${esc(max)}" data-news-time-filter="end"></label><button type="button" data-news-time-clear ${selected ? "" : "disabled"}>清除</button></div></div><div class="market-received-table-scroll" tabindex="0" role="region" aria-label="可滚动的收到新闻列表"><table aria-label="收到的新闻明细"><thead><tr><th scope="col">发布时间</th><th scope="col">新闻标题</th><th scope="col">来源</th><th scope="col">监测模块</th><th scope="col">命中关键词</th></tr></thead><tbody>${filtered.map((item) => {
