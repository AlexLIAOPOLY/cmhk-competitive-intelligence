(() => {
  ...
  function normalizedPreferenceSnapshot(item = {}) {
    ...
    return {
      ...
      news_personal_skill: item.news_personal_skill || [],
  ...
  function chatPreferenceDrawer() {
    ...
    return `<div class="drawer-backdrop" data-chat-backdrop><aside class="management-drawer chat-preference-drawer" role="dialog" aria-modal="true" aria-label="${esc(item.display_name)}的偏好与对话"><header class="drawer-header"><div><h2>${esc(item.display_name)} · 偏好与对话</h2><p>个人设置与自然语言修改记录</p></div><button class="icon-button" type="button" data-close-chat-preferences aria-label="关闭偏好与对话">${icon("close")}</button></header>
      ...
      ${item.personal_news_selection ? `<details><summary>最近一次智能选稿 · ${esc(item.personal_news_selection.updated_at)}</summary><strong>本次选稿依据</strong>${points(item.personal_news_selection.requirements)}<small>模型判断 ${number(item.personal_news_selection.candidate_count)} 条，保留 ${number(item.personal_news_selection.eligible_count)} 条候选；最终仍须完成图文审核。</small>${points((item.personal_news_selection.decisions || []).filter((row) => row.decision === "prefer").sort((a,b) => b.score-a.score).slice(0,10).map((row) => `${row.title}：${row.reason}`))}</details>` : ""}
  ...
  function compactSubscriberRows() {
    ...
    return rows.map((savedItem) => {
      ...
      <td>${conditionalSetting("news", hasNews, `${item.news_personal_skill?.length ? `<div class="personal-reading-preview"><strong>按个人阅读要求智能选稿</strong><ol>${item.news_personal_skill.slice(0,3).map((point) => `<li>${esc(point)}</li>`).join("")}</ol>${item.news_personal_skill.length > 3 ? `<small>共 ${number(item.news_personal_skill.length)} 项，完整说明见“偏好与对话”</small>` : ""}</div><details><summary>默认栏目（未设阅读要求时使用）</summary>` : ""}<div class="news-interest-group" aria-label="${esc(item.display_name)}的战略新闻兴趣板块">${newsCategoryChecks(item.news_categories)}</div>${item.news_personal_skill?.length ? "</details>" : ""}<label class="news-region-setting">新闻地域偏好<select data-subscriber-news-region aria-label="新闻地域偏好">${newsRegionOptions(item.news_region_preference)}</select></label>${item.latest_news_round ? `<small class="muted" title="${esc(JSON.parse(item.latest_news_round.detail_json || '{}').reason || '')}">本轮 ${number(item.latest_news_round.delivered_count)}/${number(item.latest_news_round.requested_count)} 条 · ${item.latest_news_round.status === 'stopped' ? '已停止补发' : item.latest_news_round.status === 'complete' ? '已完成' : item.latest_news_round.status === 'closed' ? '已集中发送，不再补发' : item.latest_news_round.status === 'exhausted' ? '候选不足' : '准备中'}</small>` : ''}`, "未订阅新闻")}</td>
  ...
  function preferenceValueText(field, value) {
    ...
    if (field === "news_personal_skill") return (value || []).join("；") || "尚未提出额外阅读要求";
  ...
  function preferenceSubmissionHistory(submissions) {
    ...
    const fieldLabels = {
      services: "订阅内容", report_mode: "报告接收方式", news_categories: "新闻兴趣板块", news_topics: "优先关注主题", news_personal_skill: "个人阅读要求",
  ...
  function render() {
    ...
    const newsSchedule = data.strategic_news_schedule || { enabled: false, times_text: "03:00 / 14:00", delivery_times: ["08:00", "18:30"], delivery_times_text: "08:00 / 18:30", timezone_label: "香港时间", dispatch_rule: "个人推送必须等对应爬虫完成；群内仍在爬完后立即发送" };
