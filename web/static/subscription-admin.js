(() => {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="admin">
      ...
      <section class="permission-panel"><div><strong>飞书人员目录：${permission.status === "ready" ? "已接通" : "受限"}</strong><span>${esc(permission.summary || "")}</span></div><div class="scope-tags">${(permission.required_scopes || []).map((item) => `<span>${esc(item.scope)}</span>`).join("")}</div><small>${esc(permission.availability_note || "")}</small></section>
