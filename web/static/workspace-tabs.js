(() => {
  ...
  function activeNewsStageNodeLabel(stageKey) {
    return ({ search: "新闻线索补缺", gate: "新闻线索补缺", ai: "新闻 AI 审核", dedupe: "新闻历史去重", write: "新增战略新闻", push: "新增战略新闻" })[stageKey] || "未定位";
  }
  ...
  function globalSchedulerLineageModel(runs, stages, attemptRuns = runs) {
    ...
    const domainNode = (key, fallbackLabel, position, variant) => {
      ...
      return {
        ...
        unit: intelligenceRun.crawl_run_id ? "个主体发布到 UI" : "",
    ...
    const mainUnit = mainRun.crawl_run_id ? "行数据源配置已处理" : "";
    ...
    const nodes = [
      ...
      { key: "insights", label: "AI战略洞察UI更新", value: intelligenceRun.crawl_run_id ? number(insightCount) : "—", unit: intelligenceRun.crawl_run_id ? "项数据洞察发布到 UI" : "", note: intelligenceRun.crawl_run_id ? `${insightGenerationLabel} ${number(insightCount)} 项数据洞察 · ${intelligenceRun.operational_summary?.pages_publish?.ok ? "页面发布已验证" : "发布待核对"}` : "当天未运行", health: modelAnalysis.fallback_used || (intelligenceRun.crawl_run_id && intelligenceHealth.key === "healthy" && (!intelligenceRun.operational_summary?.pages_publish?.ok || !insightCoverageComplete)) ? { key: "warning", label: "警告" } : intelligenceHealth, variant: "insight", position: [1385, 455], details: intelligenceRun.crawl_run_id ? [`AI数据洞察通过 ${number(insightCount)} / ${expectedInsightCount} 项；分域洞察 ${number(focusInsightCount)} / ${number(expectedFocusInsightCount)}，顶部跨库研判 ${number(discoveryInsightCount)} / ${number(expectedDiscoveryInsightCount)}；本轮${insightGenerationLabel}`, "数据洞察生成数与数据库事实数、UI发布主体数分别统计，不可互相替代", `模型 ${modelAnalysis.model || "未记录"}`, `证据指纹 ${modelAnalysis.evidence_hash || "未记录"}`, intelligenceRun.operational_summary?.pages_publish?.ok ? `业务发布 主页与公开页已验证 · 版本 ${intelligenceRun.operational_summary.pages_publish.site_version || "未记录"}` : "业务发布 未留下可核对记录"] : ["所选日期没有数据洞察与业务发布归档"], evidence: [intelligenceRun.progress_detail || intelligenceRun.status_detail, intelligenceRun.operational_summary?.pages_publish?.public_url].filter(Boolean).join("\n") || "当天未留下AI数据洞察与业务发布证据" },
    ];
    const edges = [
      ["strategic", "news-search", "启动新闻检索", "cyan"], ["news-search", "news-ai", "新闻进入审核", "cyan"], ["news-ai", "news-dedupe", "相关新闻事件", "cyan"], ["news-dedupe", "news-output", "形成新增新闻", "cyan"], ["news-output", "news-selection-agent", "学习新闻选用习惯", "cyan"], ["news-selection-agent", "app-result", "机器人勾选新闻", "cyan"], ["news-selection-agent", "weekly-result", "机器人勾选新闻", "cyan"], ["news-output", "strategic", "", "feedback"],
      ["previous-news", "news-db-signal", "读取新闻参考", "amber"], ["news-db-signal", "main", "追查数据官方原文", "handoff-down"], ["main", "agent", "提交数据证据审核", "cyan"], ["main", "news-search", "网页变化供新闻补缺", "feedback-side"], ["agent", "database-hub", "审核通过数据写库", "cyan"],
    ...
    return {
      ...
      laneLabels: [
        ...
        { label: "C｜官方数据复核、入库与 UI 发布线", position: [18, 420] },
      ],
      groups: [{ key: "databases", label: "四库主体数据发布到 UI", position: [950, 370], size: [389, 301] }],
    };
  }
  ...
  function renderDetailedRecords(nodeKey, records, relatedRuns = []) {
    ...
    const detailTitle = sourceDiscoveryNode ? "当天具体线索与官方追证入口" : selectionAgentNode ? "当天新闻自动初筛明细" : mainCrawlNode ? "当天33行固定源逐行抓取明细" : agentAuditNode ? "当天Agent逐条证据审核明细" : databaseHubNode ? "当天数据库入库与数值变化" : databaseUiNode ? "当天UI重新发布与数值变化" : nodeKey === "insights" ? "当天AI洞察明细" : "当天处理对象明细";
