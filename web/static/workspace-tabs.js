(() => {
  ...
  function legacySchedulerLineageModel(runs, stages, attemptRuns = runs) {
    ...
    const nodes = [
      { key: "strategic", label: "04:00 / 14:00 定时启动器", value: newsRun.run_status === "running" ? "运行中" : number(runs.length), unit: newsRun.run_status === "running" ? "" : "次新闻任务", note: currentStrategicStage ? `已触发 · 当前执行：${activeNewsStageNodeLabel(currentStrategicStage.key)}` : runs.length ? `只负责触发 · ${attemptRuns.length} 次任务尝试` : "当天没有触发成功的任务归档", health: strategicHealth, variant: "crawler", position: [18, 52], details: ["这个节点只负责到点启动后续新闻任务，本身不搜索网页、不审核新闻，也不保存新闻", "同一时段重试只由最终权威批次参与统计；所有任务尝试仍保留在此处供追溯", ...attemptRuns.map((run) => `${newsRunTime(run)} · ${run.scope || "战略新闻任务"} · ${run.run_status || "未记录状态"}`)], evidence: attemptRuns.map((run) => run.progress_detail || run.status_detail || run.scope).filter(Boolean).join("\n") || "当天没有战略新闻任务归档" },
    ...
    const nodePurposes = {
      strategic: "每天04:00和14:00只负责触发后续新闻任务；本节点不搜索、不审核、不保存新闻",
      ...
      "previous-news": "合并前一日04:00和14:00两个权威新闻批次，按URL或标题去重后给01:00补缺搜索作参考",
  ...
  function newsLineageFirstScreenModel(nodeKey, node, relatedRuns, records, events, incoming, outgoing) {
    ...
    const descriptions = {
      ...
      "previous-news": {
        input: "前一日最后两个已完成的权威新闻批次，通常对应04:00和14:00",
