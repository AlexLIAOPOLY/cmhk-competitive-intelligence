  ...
  function renderCompetitorResult() {
    ...
    const rows = visibleYears.map((year) => `<tr><th>${year}</th>${companies.map((company) => {
      ...
      const translated = converted?.rawUnit
        ...
        : "";
      return `<td title="${esc(cell ? [cell.period, cell.periodEnd, cell.scope, cell.basis, cell.usagePolicy, cell.note].filter(Boolean).join(" · ") : gapTitle)}">${cell ? `<strong>${esc(`${competitorComparator(cell.comparator)}${new Intl.NumberFormat("zh-CN", { maximumFractionDigits: 2 }).format(cell.value)} ${metricMeta.unitLabels?.[cell.unit] || cell.unit}`)}</strong><small>${esc([cell.period, cell.periodEnd, sourceAuthorityLabel].filter(Boolean).join(" · "))}</small>${translated}${disclosedSourceLinks}` : `<span class="competitor-missing">${gapLabel}</span>${relatedEvidence}<small>${esc(gap?.reason || "尚无审计记录")}</small>${reviewedSourceLinks}`}</td>`;
    }).join("")}</tr>`).join("");
    const fxNote = usdConversion?.lookup?.size ? `<p class="competitor-fx-note">国际运营商统一按各披露年度平均汇率换算美元；原币值不变并保留在明细中。<a href="${esc(safeUrl(usdConversion.source))}" target="_blank" rel="noreferrer">世界银行 WDI 汇率来源</a></p>` : "";
    ...
    if (nodeKey === "news-db-signal") {
      ...
      return relatedRuns.flatMap((run) => {
        ...
        return (Array.isArray(summary.signals) ? summary.signals : []).map((signal) => ({
          ...
          status: "handoff",
          resultLabel: "待追官方原文",
          reason: "该条只是公开信息线索，不是可入库数据；已交给03:00链路访问公司 IR、财报或监管披露原文，字段门禁通过后才能入库。",
