(() => {
  ...
  function globalSchedulerLineageModel(runs, stages, attemptRuns = runs) {
    ...
    const nodes = [
      ...
      { key: "news-db-signal", label: "01:00 补缺链接自动搜索", value: number(sourceDiscoverySummary.signal_count || 0), unit: "条待审核数据线索", note: "自动发现补缺链接 · 提交证据审核", health: sourceDiscoveryHealth, variant: "source", position: [300, 270], details: ["独立搜索 Agent 按四库主体与数据指标字段检索最近24小时资料", "合并前一天07:30/14:00两次战略新闻任务内容作参考", "搜索结果与新闻结果都只作数据线索，不直接成为数据库事实", "发现的链接直接提交 Agent 数据证据审核，由审核环节继续追公司 IR、财报或监管披露原文", "飞书独立子表记录查询、URL抓取、HTTP结果、入库决定与拒绝原因"], evidence: sourceDiscoverySummary.audit_path || sourceDiscoveryRun.progress_detail || "当天未留下01:00四库数据资料补缺审计" },
      { key: "main", label: "03:00 飞书固定链接抓取", value: mainValue, unit: mainUnit, note: mainRun.crawl_run_id ? `${mainCrossedDate ? "跨日完成 · " : ""}${number(configuredSourceRows)} 行飞书配置 · 本轮 ${number(crawlUrlAttemptCount)} 次网址抓取` : "当天未找到主爬虫归档", health: mainHealth, variant: "crawler", position: [300, 520], details: ["从飞书配置表读取人工维护的固定网址，再抓取网页及后续发现的官方原文", "固定链接数、配置行数和实际网址抓取次数分别统计，互不替代", ...mainDetails], evidence: mainRun.status_detail || mainRun.progress_detail || "当天未留下主爬虫运行证据" },
    ...
    const nodePurposes = {
      ...
      main: "读取飞书配置表内人工维护的固定链接，抓取网页和官方原文后提交审核",
      agent: "汇总01:00自动补缺链接与03:00飞书固定链接，核对主体、指标、期间、单位、来源和证据",
    ...
    const edges = [
      ...
      ["previous-news", "news-db-signal", "读取新闻参考", "amber"], ["news-db-signal", "agent", "自动补缺链接提交审核", "cyan"], ["main", "agent", "固定链接提交审核", "cyan"], ["main", "news-search", "网页变化供新闻补缺", "feedback-side"], ["agent", "database-hub", "审核通过数据写库", "cyan"],
