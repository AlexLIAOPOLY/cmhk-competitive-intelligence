...
class DashboardPagesPublishTests(unittest.TestCase):
    ...
    def test_build_site_is_static_sanitized_and_stable(self):
        ...
        with tempfile.TemporaryDirectory() as temp, mock.patch.object(
            ...
            self.assertIn('href="./executive-dashboard-demo.css?v=21"', html)
            ...
            self.assertIn('src="./executive-dashboard-demo.js?v=12"', html)
            self.assertIn("战略监控体系", html)
            self.assertIn("仅展示 CMHK", html)
            self.assertIn("宏观与国际", html)
            self.assertIn("政策法规", html)
            self.assertIn("基础设施与技术", html)
            self.assertIn("市场与产品", html)
            ...
            self.assertIn("monitoringViews", script)
            self.assertIn("refreshStrategicSignals", script)
            self.assertNotIn("benchmark", script.lower())
            self.assertNotIn("HKT", html)
            self.assertNotIn("3香港", html)
            self.assertNotIn("SmarTone", html)
            self.assertNotIn("HKBN", html)
            self.assertIn("competitorTerms", script)
            ...
            self.assertFalse((first / "executive-responsive-hardening.css").exists())
