"""Handle CMHK incident-card callbacks and write the clicking user to Feishu Sheets.

This listener is intentionally separate from the project monitor.  It consumes
only ``card.action.trigger`` events, accepts only callback values generated by
the checked-in alert card, verifies the original message against the monitor's
delivery ledger, and updates only the human-owned M/N columns in the error
worksheet.  It never sends chat messages.
"""
...
class CardActionHandler:
    ...
    def handle_event(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(processed.get(event_id), dict) and processed[event_id].get("status") == "completed":
