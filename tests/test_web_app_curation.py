...
class ReportFileNameTests(unittest.TestCase):
    ...
    def test_dashboard_uses_four_domain_intelligence_with_drillthrough(self) -> None:
        ...
        self.assertIn('href="/static/styles.css?v=295"', html)
...
class HomepageTickerAndTabRegressionTests(unittest.TestCase):
    ...
    def test_homepage_compact_desktop_prevents_text_overlap(self) -> None:
        ...
        self.assertIn('grid-template-columns: minmax(82px, .75fr) minmax(36px, 1fr) minmax(82px, auto);', leadership_styles)
...
class IntelligenceEntityScrollTests(unittest.TestCase):
    def test_financial_results_stay_in_drawer_and_run_log_without_an_extra_focus_tab(self) -> None:
        ...
        self.assertIn('const interpretationLabel = "AI 战略解读"', app)
        self.assertIn('const aiAnalysis = focus.ai_summary?.analysis;', app)
        self.assertNotIn('focus.ai_summary?.analysis || focus.insight', app)
        self.assertIn('AI 分析待生成', app)
    ...
    def test_domain_title_is_display_only_and_relation_title_regenerates_discovery(self) -> None:
        ...
        self.assertIn('const relationAfter = { ...relationBefore, ...data };', app)
        self.assertIn('payload.relations[index] = relationAfter;', app)
        self.assertIn('relationRefreshState.get(index)?.message || "生成失败，点击标题重试"', app)
        self.assertNotIn('? safe(relationRefreshState.get(index)?.message || "本次未生成新内容")', app)
