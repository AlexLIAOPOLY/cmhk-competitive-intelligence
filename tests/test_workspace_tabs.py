...
class WorkspaceTabsTests(unittest.TestCase):
    ...
    def test_news_lineage_uses_semantic_health_colours_instead_of_node_type_colours(self):
        ...
        self.assertIn('{ key: "healthy", label: "已启动" }', SCRIPT)
        self.assertIn('value: strategicTriggered ? "已启动" : "—", unit: ""', SCRIPT)
        self.assertIn('`启动已完成 · ${attemptRuns.length} 次任务尝试`', SCRIPT)
        self.assertIn('启动成功后即显示绿色“已启动”', SCRIPT)
    ...
    def test_news_lineage_locates_running_work_on_one_specific_node(self):
        ...
        self.assertIn('`启动已完成 · 后续执行：${activeNewsStageNodeLabel(currentStrategicStage.key)}', SCRIPT)
