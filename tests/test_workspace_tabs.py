...
class WorkspaceTabsTests(unittest.TestCase):
    ...
    def test_news_module_maps_all_periodic_crawlers_into_four_database_updates(self):
        ...
        self.assertIn('workspace-tabs.css?v=159', INDEX)
        self.assertIn('workspace-tabs.js?v=185', INDEX)
        ...
        self.assertIn('agent: "逐条审核C1b形成的候选数据；不把01:00链接数和03:00抓取数相加"', SCRIPT)
        ...
        self.assertIn('["main", "fact-extract", "正文", "cyan"]', SCRIPT)
        self.assertIn('["fact-extract", "agent", "送审", "cyan"]', SCRIPT)
        ...
        self.assertIn('label: "C｜证据审核主链：两类链接 → 抓取 → 提取 → 审核 → 入库 → UI", position: [18, 430]', SCRIPT)
        self.assertNotIn('note: "仅官方原文 + 字段门禁通过才写入"', SCRIPT)
        self.assertIn('primary: true, position: [700, 392]', SCRIPT)
        self.assertIn('position: [920, 415]', SCRIPT)
        self.assertIn('domainNode("local", "本地运营商", [1130, 410]', SCRIPT)
        self.assertIn('domainNode("international", "国际运营商", [1295, 410]', SCRIPT)
        self.assertIn('domainNode("cloud", "全球云厂商", [1130, 535]', SCRIPT)
        self.assertIn('domainNode("mainland", "内地运营商", [1295, 535]', SCRIPT)
        self.assertIn('position: [1525, 455]', SCRIPT)
        self.assertIn('label: "审核通过数据按主体类型写入四库并发布到 UI", note: "四库已发布数据汇总生成右侧 AI 战略洞察", position: [1090, 370], size: [389, 301]', SCRIPT)
