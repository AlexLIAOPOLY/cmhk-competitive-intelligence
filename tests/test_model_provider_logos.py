...
class ModelProviderLogoTests(unittest.TestCase):
    ...
    def test_picker_keeps_text_accessible_and_logo_decorative(self) -> None:
        ...
        self.assertNotIn(".chat-model-provider-logo.is-kimi { background:", styles)
        kimi = (ASSETS / "kimi.svg").read_text(encoding="utf-8")
        self.assertEqual(kimi.count('fill="#1783FF"'), 2)
        self.assertNotIn('fill="#fff"', kimi)
