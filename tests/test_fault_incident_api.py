...
class FaultIncidentApiTests(unittest.TestCase):
    def test_handler_reconciliation_timeout_is_deferred_without_traceback(self) -> None:
        with (
            ...
            handler.return_value.sync_handlers_from_sheet.side_effect = (
                web_app.subprocess.TimeoutExpired(["lark-cli"], 30)
            )
