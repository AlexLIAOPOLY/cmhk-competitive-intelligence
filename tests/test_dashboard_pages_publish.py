...
class DashboardPagesPublishTests(unittest.TestCase):
    ...
    def test_monitoring_dashboard_shows_all_local_operator_metrics_without_tabs(self):
        ...
        self.assertIn('chartTypes: ["bar", "lollipop", "bar", "lollipop", "bar", "bar"]', script)
        self.assertIn('key: "reach", number: "03", title: "渠道与品牌触达层", metricCount: 2, chartTypes: ["bar", "bar"]', script)
        self.assertIn('chartTypes: ["bar", "lollipop", "diverging"]', script)
        ...
        self.assertIn(".comparison-grid { grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; }", style)
        ...
        self.assertIn("network-context-v1.webp", style)
        self.assertIn("mobile-context-v1.webp", style)
        self.assertIn("store-context-v1.webp", style)
        self.assertIn("financial-growth.webp", style)
        self.assertIn("opacity: .12", style)
        self.assertIn("opacity: .16", style)
        self.assertIn('"SF Pro Text", "PingFang SC"', style)
        self.assertIn("font: 600 13.5px/1.25 var(--font-ui)", style)
        self.assertIn("font-size: clamp(14px, .74vw, 17px)", style)
