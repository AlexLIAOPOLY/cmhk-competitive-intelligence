...
class DashboardPagesPublishTests(unittest.TestCase):
    ...
    def test_monitoring_dashboard_shows_all_local_operator_metrics_without_tabs(self):
        ...
        self.assertIn('period: "2026上半年"', script)
        ...
        self.assertNotIn(".comparison-metric-card > header::before", style)
        self.assertIn(".comparison-metric-card h3::after", style)
        self.assertIn("width: 22px", style)
        self.assertIn("background: linear-gradient(90deg, #e4faff 0%, var(--panel-accent) 52%, transparent 100%)", style)
        ...
        self.assertIn('chartTypes: ["column", "bar", "column", "lollipop", "column", "bar"]', script)
        self.assertIn('key: "reach", number: "03", title: "渠道与品牌触达层", metricCount: 2, chartTypes: ["donut", "column"]', script)
        self.assertIn('chartTypes: ["bar", "radial", "bar"]', script)
        ...
        self.assertIn('left: "32%", top: "42%", padding: 0, textAlign: "center"', script)
        ...
        self.assertNotIn("chart-hit-area", script)
        self.assertNotIn("chart-ring-hit-area", script)
        ...
        self.assertIn("cursor: pointer; pointer-events: visiblePainted", style)
        self.assertIn("cursor: pointer; pointer-events: visibleStroke", style)
        self.assertNotIn("cursor: help", style)
        self.assertIn(".comparison-chart text,", style)
        self.assertNotIn("network-context-v1.webp", style)
        self.assertNotIn("mobile-context-v1.webp", style)
        self.assertNotIn("store-context-v1.webp", style)
        self.assertNotIn("financial-growth.webp", style)
        self.assertIn('"SF Pro Text", "PingFang SC"', style)
        ...
        self.assertNotIn(".panel-reach .comparison-chart text", style)
        ...
        self.assertNotIn("data-comparison-metric", script)
        self.assertNotIn('role="tab"', script)
    ...
    def test_static_intelligence_snapshot_keeps_frontend_interactions_without_api(self):
        static_dir = Path(__file__).resolve().parents[1] / "web" / "static" / "intelligence-public"
        html = (static_dir / "index.html").read_text(encoding="utf-8")
        script = (static_dir / "intelligence.js").read_text(encoding="utf-8")
        builder = (Path(__file__).resolve().parents[1] / "scripts" / "build_intelligence_static_snapshot.js").read_text(encoding="utf-8")
    ...
    def test_build_site_publishes_unified_read_only_homepage(self):
        ...
        with tempfile.TemporaryDirectory() as temp, mock.patch.object(
            ...
            self.assertLess(
                html.index('src="./static/workspace-tabs.js?v=public-8"'),
                html.index('src="./static/app.js?v=319"'),
            )
