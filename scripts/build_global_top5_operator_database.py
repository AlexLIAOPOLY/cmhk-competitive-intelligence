...
METRICS = {
    ...
    "fixed_broadband_access_ports": ("固网宽带接入端口", "million_ports"),
...
SOURCES.update({
    ...
    **{
        ...
        for year in (2016, 2017, 2020, 2022, 2023, 2024, 2025)
    },
    ...
    "china_unicom_transcript_2020": {
        "source_id": "china_unicom_transcript_2020",
        "operator_id": "china_unicom",
        "year": 2020,
        "label": "China Unicom 2020 annual results analyst meeting transcript",
        "url": "https://www.chinaunicom.com.hk/en/ir/transcript/transcript_20ar.pdf",
...
SOURCES["bharti_airtel_ar_2025"]["evidence"] = {
    ...
    "mobile_arpu": {
        "value": 245,
        "unit": "INR_per_user_month",
        "locator": "India mobile services review; FY2025 industry-leading ARPU",
    },
}
...
SOURCES["reliance_jio_ar_2017"].setdefault("evidence", {})["mobile_dou"] = {
    "value": 10,
    "unit": "GB_per_user_month",
    "locator": "Digital Services customer outcome; average consumption on Jio is 10GB/month/user",
}
SOURCES["reliance_jio_ar_2018"].setdefault("evidence", {}).update(
    {
        "mobile_arpu": {
            "value": 137,
            "unit": "INR_per_user_month",
            "locator": "Digital Services FY2018 exit-quarter KPI; industry-leading ARPU",
        },
        "mobile_dou": {
            "value": 9.7,
            "unit": "GB_per_user_month",
            "locator": "Digital Services FY2018 exit-quarter average data consumption",
        },
    }
)
...
_CU_FIXED_BROADBAND_ACCESS_PORTS = {
...
_CU_FIXED_BROADBAND_SUBSCRIBER_SOURCES = {
    ...
    2021: ["china_unicom_results_2021", "china_unicom_dec_ops_2021", "china_unicom_csr_2021"],
    2022: ["china_unicom_results_2022", "china_unicom_csr_2023", "china_unicom_csr_2024"],
}
...
_CU_FIXED_BROADBAND_ACCESS_PORT_SOURCES = {
    year: [
        f"china_unicom_csr_{year}",
...
_CU_5G_PACKAGE_SUBSCRIBER_SOURCES = {
    2020: ["china_unicom_ar_2020", "china_unicom_results_2020", "china_unicom_transcript_2020"],
}
...
for _source_id in _CU_5G_PACKAGE_SUBSCRIBER_SOURCES[2020]:
    _bind_cu_historical_evidence(
        _source_id,
        year=2020,
        metric_key="5g_package_subscribers",
        value=70.83,
        unit="million_subscribers",
        locator="FY2020 year-end 5G package subscribers; exact 70.83 million disclosure",
    )
    ...
    *,
    ...
    for year, value in values.items():
        ...
        ROWS.append({
            ...
            "year": year,
            "period": f"FY{year}",
            "period_end": f"{year + int(OPERATORS[operator_id].get('period_end_year_offset') or 0)}-{OPERATORS[operator_id]['fiscal_year_end']}",
            "grain": "annual",
            "metric_key": metric_key,
            "metric_zh": metric_zh,
            "value": value,
...
add_series("china_unicom", "5g_package_subscribers", {2020:70.83,2021:154.93,2022:212.73,2023:259.64,2024:290.44}, scope="5G package subscribers; disclosure replaced by network-user measure in 2025", source_ids={**paired("china_unicom", list(range(2020,2025))), **_CU_5G_PACKAGE_SUBSCRIBER_SOURCES}, note="FY2020 uses the H-share annual report, annual-results presentation and separately published analyst-meeting transcript, each stating the exact 70.83 million year-end value. Later years remain below the strict gate until three documents carrying the same precision are bound; rounded narrative is not counted.")
...
add_series("china_unicom", "fixed_broadband_subscribers", {2016:75.236,2017:76.539,2018:80.880,2019:83.478,2020:86.095,2021:95.046,2022:103.630,2023:113.42,2024:122.26,2025:129.87}, scope="fixed-line broadband billing subscribers", source_ids={**paired("china_unicom", cu_years), **_CU_FIXED_BROADBAND_SUBSCRIBER_SOURCES}, note="FY2016-FY2022 retain the exact thousand-subscriber precision repeated across three separate official documents. FY2022 uses the annual-results presentation plus the FY2023 and FY2024 sustainability-report comparative tables. FY2023-FY2025 remain approximate or below the strict three-document gate.")
...
add_series("bharti_airtel", "mobile_arpu", {2025:245}, unit="INR_per_user_month", scope="India mobile services ARPU; FY2025 year-end/exit-quarter disclosure, not consolidated group ARPU", basis="year_end", source_ids={2025:["bharti_airtel_ar_2025"]}, note="FY2025 is displayed from one direct official annual-report source under the user-approved one-source display rule; source count and India-mobile scope remain explicit.")
...
add_series("reliance_jio", "mobile_arpu", {2016:None,2017:None,2018:137,2019:126.2,2020:130.6,2021:138.2,2022:167.6,2023:178.8,2024:181.7,2025:206.2}, unit="INR_per_user_month", scope="exit-quarter ARPU, not full-year average", basis="exit_quarter", source_ids=override_sources(override_sources(override_sources(override_sources(override_sources(override_sources(override_sources(override_sources({y:jio_sources.get(y,[f"reliance_jio_ar_{y}"]) for y in YEARS}, 2018, ["reliance_jio_ar_2018"]), 2019, jio_2019_operating_three), 2020, jio_2020_operating_three), 2021, jio_2021_operating_three), 2022, jio_2022_operating_sources), 2023, jio_2023_operating_three), 2024, ["jio_2024_q4", "jio_2025_media_release", "jio_2024_factsheet"]), 2025, ["jio_2025_q4", "jio_2025_media_release", "jio_2025_factsheet"]), note="FY2017 remains blank because the introductory service period did not disclose a comparable paid exit-quarter ARPU. FY2018 INR137 is retained from the official annual report even though only one exact official document is bound.")
add_series("reliance_jio", "mobile_dou", {2016:None,2017:10,2018:9.7,2019:10.9,2020:11.3,2021:13.3,2022:19.7,2023:23.1,2024:28.7,2025:33.6}, scope="exit-quarter average data consumption per user per month", basis="exit_quarter", source_ids=override_sources(override_sources(override_sources(override_sources(override_sources(override_sources(override_sources(override_sources(override_sources({y:jio_sources.get(y,[f"reliance_jio_ar_{y}"]) for y in YEARS}, 2017, ["reliance_jio_ar_2017"]), 2018, ["reliance_jio_ar_2018"]), 2019, jio_2019_operating_three), 2020, jio_2020_operating_three), 2021, jio_2021_operating_three), 2022, ["reliance_jio_ar_2022", "jio_2022_q4", "jio_2023_q4"]), 2023, ["reliance_jio_ar_2023", "jio_2023_q4", "jio_2024_q4"]), 2024, ["reliance_jio_ar_2024", "jio_2024_q4", "jio_2025_q4"]), 2025, ["reliance_jio_ar_2025", "jio_2025_q4", "jio_2025_media_release", "jio_2025_factsheet"]), note="FY2017 10GB and FY2018 9.7GB are retained from their official annual reports with limited-source labels rather than omitted for having fewer than three exact document bindings.")
...
def build_coverage(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    required = {
        ...
        "china_unicom": ["mobile_subscribers","5g_network_subscribers","fixed_broadband_subscribers","fixed_broadband_access_ports","mobile_arpu","mobile_dou","4g_base_stations","5g_base_stations"],
        ...
        "bharti_airtel": ["total_customers","revenue","ebitda","net_profit","capex","mobile_arpu","network_towers","total_data_traffic"],
        ...
        "verizon": ["revenue", "net_profit", "reported_mobile_connections"],
