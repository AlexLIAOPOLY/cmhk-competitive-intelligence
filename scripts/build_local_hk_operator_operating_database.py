    ...
    for year, value in values.items():
        ...
        row_status = (
            (
                ...
                if resolved_gap_reason_code == "not_applicable_business_scope"
                else "not_applicable_precommercial"
                if resolved_gap_reason_code == "precommercial_kpi_not_defined"
                else "scope_not_comparable"
                if resolved_gap_reason_code == "metric_definition_not_comparable"
...
add_series("hkt", "5g_customers", {year: None for year in range(2016, 2020)}, scope="commercial post-paid 5G customer base", source_ids=annual_sources("hkt", list(range(2016, 2020))), note="Commercial 5G service had not launched; the cell is explicitly not applicable before commercial launch and is not stored as zero.", gap_reason_code="precommercial_kpi_not_defined")
...
add_series("hkt", "5g_penetration", {year: None for year in range(2016, 2020)}, scope="commercial 5G penetration within post-paid customer base", source_ids=annual_sources("hkt", list(range(2016, 2020))), note="Commercial 5G did not exist in Hong Kong before 2020; the cell is explicitly not applicable and is not stored as 0%.", gap_reason_code="precommercial_kpi_not_defined")
...
add_series("three_hk", "5g_penetration", {year: None for year in range(2016, 2020)}, scope="commercial 5G penetration within post-paid base", source_ids=annual_sources("three_hk", list(range(2016, 2020))), note="3HK launched commercial 5G in April 2020; the earlier cells are explicitly not applicable and are not stored as 0%.", gap_reason_code="precommercial_kpi_not_defined")
...
add_series("three_hk", "5g_customers", {year: None for year in range(2016, 2020)}, scope="commercial 5G customer base", source_ids=annual_sources("three_hk", list(range(2016, 2020))), note="Commercial 5G service had not launched; the earlier cells are explicitly not applicable and are not stored as zero.", gap_reason_code="precommercial_kpi_not_defined")
...
add_series("smartone", "5g_penetration", {year: None for year in range(2016, 2020)}, scope="commercial 5G penetration among post-paid MNO customers", source_ids=annual_sources("smartone", list(range(2016, 2020))), note="SmarTone launched commercial 5G in May 2020; the earlier cells are explicitly not applicable and are not stored as 0%.", gap_reason_code="precommercial_kpi_not_defined")
...
add_series("smartone", "5g_customers", {year: None for year in range(2016, 2020)}, scope="commercial 5G customer base", source_ids=annual_sources("smartone", list(range(2016, 2020))), note="Commercial 5G service had not launched; the earlier cells are explicitly not applicable and are not stored as zero.", gap_reason_code="precommercial_kpi_not_defined")
...
add_series("hkbn", "mobile_postpaid_customers", {2016: None}, scope="activated mobile subscriptions under HKBN's MVNO service", source_ids=hkbn_sources, note="FY2016 ended before HKBN's September 2016 consumer mobile launch; the cell is explicitly precommercial and is not stored as zero.", gap_reason_code="precommercial_kpi_not_defined")
add_series("hkbn", "mobile_postpaid_customers", {2017: .147, 2018: .265, 2019: .277, 2020: .275, 2021: .254, 2022: .241, 2023: .239, 2024: .217, 2025: .181}, scope="activated mobile subscriptions under HKBN's MVNO service", source_ids=hkbn_sources, note="FY2017-FY2025 are the company's disclosed activated/mobile subscriptions; they are not network-owner SIM totals.")
    ...
    if metric_key.startswith("5g_") or metric_key in {"5g_customers", "5g_penetration", "5g_population_coverage", "5g_base_stations"}:
        if year < 2020:
            return (
                ...
                f"香港商业5G于2020年推出；FY{year}官方材料没有把{metric}定义为可复用年度KPI。该格明确标为商用前不适用，不以0表示。",
            )
